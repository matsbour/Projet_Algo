...
int main(int argc, char* argv[])//Main du projet finale, prend trois parametres
{
	...
	case(4):
	{
		if(atoi(argv[3]) == 1){scoring_algorithm = new Smith_Waterman("BLOSUM62",prot_query,DEFAULT_GAP_OPENER, DEFAULT_GAP_EXTENSION);}
		
		//chemin différent de Blosum 62
		else{scoring_algorithm = new Smith_Waterman(argv[3],prot_query,DEFAULT_GAP_OPENER, DEFAULT_GAP_EXTENSION);}
	}
	break;
	case(5):
	{
		if(atoi(argv[3]) == 1)
		{
			if(atoi(argv[4])<0){scoring_algorithm = new Smith_Waterman("BLOSUM62",prot_query,DEFAULT_GAP_OPENER, DEFAULT_GAP_EXTENSION);}
			
			//chemin différent de Blosum 62 et différente valeur de gap opener
			else{scoring_algorithm = new Smith_Waterman("BLOSUM62",prot_query,DEFAULT_GAP_OPENER, DEFAULT_GAP_EXTENSION);}
		}
		else
		{
			if(atoi(argv[4])<0){scoring_algorithm = new Smith_Waterman(argv[3],prot_query,DEFAULT_GAP_OPENER, DEFAULT_GAP_EXTENSION);}
			else{scoring_algorithm = new Smith_Waterman(argv[3],prot_query,atoi(argv[4]), DEFAULT_GAP_EXTENSION);}
		}
	...
	case(6):
	{
		if(atoi(argv[3]) == 1)
		{
			if(atoi(argv[4])<0)
			{
				if(atoi(argv[5])<0){scoring_algorithm = new Smith_Waterman("BLOSUM62",prot_query,DEFAULT_GAP_OPENER, DEFAULT_GAP_EXTENSION);}
				
				//différente valeur de gap extension
				else{scoring_algorithm = new Smith_Waterman("BLOSUM62",prot_query,DEFAULT_GAP_OPENER, atoi(argv[5]));}
			}
			else
			{
				if(atoi(argv[5])<0){scoring_algorithm = new Smith_Waterman("BLOSUM62",prot_query,atoi(argv[4]), DEFAULT_GAP_EXTENSION);}
				
				//différentes valeurs de gap opener et gap extension
				else{scoring_algorithm = new Smith_Waterman("BLOSUM62",prot_query,atoi(argv[4]), atoi(argv[5]));}
			}
		}
		else
		{
			if(atoi(argv[4])<0)
			{
				if(atoi(argv[5])<0){scoring_algorithm = new Smith_Waterman(argv[3],prot_query,DEFAULT_GAP_OPENER, DEFAULT_GAP_EXTENSION);}
				
				//différent chemin de blosum 62 et différent gap extension
				else{scoring_algorithm = new Smith_Waterman(argv[3],prot_query,DEFAULT_GAP_OPENER, atoi(argv[5]));}
			}
			else
			{
				if(atoi(argv[5])<0){scoring_algorithm = new Smith_Waterman(argv[3],prot_query,atoi(argv[4]), DEFAULT_GAP_EXTENSION);}
				
				//les 3 paramètres sont différents de ceux par défaut 
				else{scoring_algorithm = new Smith_Waterman(argv[3],prot_query,atoi(argv[4]), atoi(argv[5]));}
			}
		}
	}
	break;
